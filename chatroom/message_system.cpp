...
void pushMessage(string message) {
    ...
    ofstream file;
    file.open("messages.csv", ios_base::app);//added at the end of the existing content
    file << message;
    file.close();
...
void viewAllMessages() {
    ...
    vector<string> users = {"abhi"};
    map<string, string> userColors;

    string outline = "------------------------------------------------------------------------------------";
    file.open("messages.csv", ios::in);//readmode
...
void deleteAll() {
    ofstream file;
    file.open("messages.csv", ios::out | ios::trunc);//writemode
    file.close();
...
int main() {
    ...
    cout << "Welcome to the SKIN Messaging System"<< endl;
    user.request();
    user.checkIfUser();
